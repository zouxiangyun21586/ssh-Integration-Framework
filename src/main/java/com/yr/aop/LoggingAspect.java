...
public class LoggingAspect {
     ...
     * com.yr..*.*(..)所有方法执行  
     * com.yr.service.*.*(..)指定包下所有类执行
     */
    //这里只是一个规则　
    @Pointcut("execution(* com.yr.service.DepartmentServiceImpl.insert(..))") // 指定哪个类的哪个方法
    ...
    public void afterThrowing(JoinPoint joinPoint, Exception e) { // 如果不进入查看环绕通知是否开启,环绕通知会将异常处理(异常通知是特殊的)
     ...
     * 在前方法执行前执行，在后方法执行后执行
     */
     //@Around("declareJointPointExpression()") // 开启会导致异常通知不会进入,因为在这里处理了
